<template>
  <v-container width="800px" fluid>
    <v-card max-width="auto" class="mx-auto" variant="text">
      <v-card-text>
        <v-form :disabled="isSubmitting" @submit.prevent="submit" @keydown.enter.prevent>
          ...
          <section class="my-6">
            <v-row align="center" justify="center">
              ...
              <v-col cols="3">
                ...
                <v-img
                  v-if="charasheetsix.image"
                  :src="charasheetsix.image"
                  alt="調查員圖像"
                  max-height="200"
                  max-width="200"
                  contain
                  @click="triggerFileAgent"
                ></v-img>
              </v-col>
              <v-col cols="3">
                <label class="text-h6">上傳調查員圖像</label>
                <VueFileAgent
                  ref="fileAgent"
                  v-model="fileRecords"
            ...
            <v-table class="border">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>{{ computedValues.totallisten }}</td>
                </tr>
                <tr>
                  <td>圖書館使用</td>
                  <td>{{ fixedlibraryuse }}</td>
                  <td>
                    <v-text-field
                      v-model="oplibraryuse.value.value"
                      type="number"
                      density="compact"
                      variant="outlined"
                      :hide-spin-buttons="true"
                  ...
                  <td>
                    <v-text-field
                      v-model="iplibraryuse.value.value"
                      type="number"
                      density="compact"
                      variant="outlined"
                      :hide-spin-buttons="true"
                  ...
                  <td>
                    <v-text-field
                      v-model="gplibraryuse.value.value"
                      type="number"
                      density="compact"
                      variant="outlined"
                      :hide-spin-buttons="true"
                  ...
                  <td>{{ computedValues.totallibraryuse }}</td>
                </tr>
              </tbody>
            </v-table>
          </section>
          ...
          <section class="my-6">
            <v-row justify="start" align="center">
              <v-col>
                <label class="text-h4">背景故事</label>
              </v-col>
            </v-row>
          </section>
          <section class="my-6">
            <v-row>
              <v-col>
                <v-textarea v-model="background.value.value" type="text" variant="outlined" auto-grow></v-textarea>
              </v-col>
            </v-row>
          </section>
          <v-divider></v-divider>
...
const getChara = async () => {
  try {
    ...
    oplisten.setValue(charasheetsix.value.oplisten)
    iplisten.setValue(charasheetsix.value.iplisten)
    gplisten.setValue(charasheetsix.value.gplisten)

    oplibraryuse.setValue(charasheetsix.value.oplibraryuse)
    iplibraryuse.setValue(charasheetsix.value.iplibraryuse)
    gplibraryuse.setValue(charasheetsix.value.gplibraryuse)

    background.setValue(charasheetsix.value.background)
  } catch (error) {
    console.log(error)
    router.push('/')
  }
...
const schema = yup.object({
  ...
  oplisten: yup.number().integer().default(0),
  iplisten: yup.number().integer().default(0),
  gplisten: yup.number().integer().default(0),

  oplibraryuse: yup.number().integer().default(0),
  iplibraryuse: yup.number().integer().default(0),
  gplibraryuse: yup.number().integer().default(0),
...
const oplisten = useField('oplisten')
const iplisten = useField('iplisten')
const gplisten = useField('gplisten')
...
const fields = [
  ...
  { label: '精神障礙', model: mental_disorders },
  { label: '性別', model: sex },
  { label: '年齡', model: age },
]
...
const fixedlisten = 25
const fixedlibraryuse = 25
...
const computedValues = computed(() => ({
  ...
  moveoccupationpoints:
    (parseFloat(opspothidden.value.value) || 0) +
    (parseFloat(oplisten.value.value) || 0) +
    (parseFloat(oplibraryuse.value.value) || 0),
  interestpoints: parseFloat(int.value.value) * 10 || 0,
  moveinterestpoints:
    (parseFloat(ipspothidden.value.value) || 0) +
    (parseFloat(iplisten.value.value) || 0) +
    (parseFloat(iplibraryuse.value.value) || 0),
  totalspothidden:
    fixedspothidden +
    (parseFloat(opspothidden.value.value) || 0) +
    (parseFloat(ipspothidden.value.value) || 0) +
  ...
  totallisten:
    ...
    (parseFloat(oplisten.value.value) || 0) +
    (parseFloat(iplisten.value.value) || 0) +
    (parseFloat(gplisten.value.value) || 0),
  totallibraryuse:
    fixedlibraryuse +
    (parseFloat(oplibraryuse.value.value) || 0) +
    (parseFloat(iplibraryuse.value.value) || 0) +
    (parseFloat(gplibraryuse.value.value) || 0),
}))
...
const submit = handleSubmit(async (values) => {
  ...
  try {
    ...
    fd.append('oplisten', values.oplisten)
    fd.append('iplisten', values.iplisten)
    fd.append('gplisten', values.gplisten)

    fd.append('oplibraryuse', values.oplibraryuse)
    fd.append('iplibraryuse', values.iplibraryuse)
    fd.append('gplibraryuse', values.gplibraryuse)
